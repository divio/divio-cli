...
def divio_project(request):
    ...
    if  test_project_name is None:
        pytest.skip("project name for the test is not supplied. Please use `--test_project_name <name>` to specify one .")   
    ...
    test_project_dir_full_path = os.path.join(test_project_directory, test_project_name)

    if not os.path.exists(test_project_dir_full_path):
        process = subprocess.Popen(["divio", "project", "setup", test_project_name], cwd=test_project_directory)
