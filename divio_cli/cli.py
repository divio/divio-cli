...
def login_token_helper(ctx, value):
    if not value:
        ...
        if "".join(set(value)) == "\x16":
            click.secho(
                "\nThe access token provided indicates that your terminal might not have copying/pasting shortcuts such as Ctrl+C/Ctrl+V enabled. Make sure that you can use those shortcuts before you provide your access token as the input will be hidden for security reasons.",
                fg="yellow",
            )
    return value
