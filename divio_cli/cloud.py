...
class CloudClient(object):
    ...
    def get_service_instance(
        ...
        try:
            ...
            if results:
                ...
                if len(results) != 1:
                    click.secho(
                        f"Error in finding service instance of type: {instance_type} (prefix={prefix}). Got {len(results)} results.",
                        fg="red",
                        err=True,
                    )
                    sys.exit(1)
                ...
                click.secho(
                    f"No service instances of type {instance_type} found for this environment.",
                    fg="yellow",
                )
                sys.exit(0)
            ...
            click.secho(
                "Error in fetching service instances.",
                fg="red",
                err=True,
            )
            sys.exit(1)

    def create_backup(self, environment_uuid, service_instance_uuid):
        request = api_requests.CreateBackupRequest(
            self.session,
            data={
                "environment": environment_uuid,
                "services": [service_instance_uuid],
                "trigger": "MANUAL",  # TODO remove
            },
        )
        return request()
    ...
    def create_backup_download(
        ...
        response = api_requests.CreateBackupDownloadRequest(
            ...
            data={
                ...
                "trigger": "MANUAL",  # TODO use DOWNLOAD
            },
        )()
        ...
        if not backup_download_uuid:
            click.secho(
                "Error in creating backup download.",
                fg="red",
                err=True,
            )
            sys.exit(1)
        ...
        try:
            ...
            if results:
                ...
                click.secho(
                    f"Could not find service instance backup download for backup download {backup_download_uuid}.",
                    fg="red",
                )
                sys.exit(1)
    ...
    def get_backup_download_service_instance(
        ...
        request = api_requests.GetBackupDownloadServiceInstanceRequest(
            self.session,
            url_kwargs={
